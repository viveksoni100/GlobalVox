...
This project represents a light modification of the original Spring JPetStore from
mid-November 2004 (but using Spring 1.2.4 and current libs), to show how Tapestry
(v3.0.3) may be integrated with Spring, as described in chapter 14.

The original JSP view layer is still there, but an additional Tapestry page to view
an item has been added. To access this Tapestry page, start the HSQL db via the
server.bat script in db/hsqldb, build and deploy the app, and access the following
url:
  http://localhost:8080/jpetstore/app
(assuming the app is deployed to a servlet engine running on port 8080 on localhost).
An item number such as EST-1 may be entered to view that item.

The rest of this readme file is the original contents from the original JPetstore.
